 ...
 * Fully optimized to remove all unused local variables, preventing any TypeScript or ESLint warnings.
 */
export function sha256(ascii: string): string {
  ...
  let padStr = '';
  const targetLen = paddedAscii[lengthProperty] + (paddingNeeded >= 8 ? paddingNeeded : paddingNeeded + 64) - 8;
  ...
  words[words[lengthProperty]] = (asciiLength / Math.pow(2, 32)) | 0;
